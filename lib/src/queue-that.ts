...
import { createGlobalVariableAdapter, GlobalVariableAdapter } from './global-variable-adapter';
...
export function createQueueThat(options: QueueThatOptions) {
    ...
    let storageAdapter: LocalStorageAdapter | GlobalVariableAdapter = createLocalStorageAdapter(options.label);
    ...
    function checkQueue() {
        ...
        if (lastActiveQueue.id !== queueId) console.info('Switching active queue to ' + queueId);

        storageAdapter.setActiveQueue(queueId);

        const batch = storageAdapter.getQueue().slice(0, /*@Changes*/ options.batchSize ?? DEFAULT_BATCH_SIZE);
        if (batch.length === 0) {
            return;
        }
